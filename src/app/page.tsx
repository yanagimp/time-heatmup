...
export default function Home() {
  ...
  useEffect(() => {
    fetch("https://script.google.com/macros/s/AKfycbyuHgoG7YzCjTtbicvvtgn6vZs419f4b1HUqQlveFVCaSM3cK10r14SDFpor8a4PNVb/exec")
      .then((res) => res.json())
      .then((d) => {
        const converted: HeatmapEntry[] = d.map((entry: { date: string; minutes: number }) => ({          date: entry.date,
  ...
  return (
    <div className="p-4">
      ...
      <CalendarHeatmap
        ...
        endDate={new Date()}
        values={data}
        tooltipDataAttrs={(value: HeatmapEntry) => {
          if (!value?.date) return {};
          const h = Math.floor(value.count);
          const m = Math.round((value.count - h) * 60);
          return {
            "data-tooltip-id": "heatmap-tooltip",
            "data-tooltip-content": `${value.date}: ${h}h ${m}m`,
          };
        }}
                
        classForValue={(value: HeatmapEntry) => {
